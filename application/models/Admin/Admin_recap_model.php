...
class Admin_recap_model extends CI_Model
{
  ...
  public function getAcademic($id)
  {
    ...
    return $this->db->get($this->tableAcademicYear);
  }
  ...
  public function getDataLecturerByPeriod($academic = null)
  {
    return $this->db->query("SELECT academic_year.name as academic_year, lecture.nip, lecture.name as lecture_name, student.fullname, student.npm, prodi.name as prodi_name, company.name as company_name, registration.group_status FROM registration JOIN lecture ON lecture.id = registration.lecture_id JOIN student ON student.id = registration.student_id JOIN company ON company.id = registration.company_id JOIN prodi ON prodi.id = student.prodi_id JOIN academic_year ON academic_year.id = registration.academic_year_id");
  }

  public function getDataLecturerGroupBy($academic = null)
  {
    return $this->db->query("SELECT academic_year.name as academic_year, lecture.prodi_id, prodi.name as prodi_name, lecture.nip, lecture.name as lecture_name, student.fullname, student.npm, company.name as company_name, registration.group_status FROM registration JOIN lecture ON lecture.id = registration.lecture_id JOIN student ON student.id = registration.student_id JOIN company ON company.id = registration.company_id JOIN prodi ON prodi.id = lecture.prodi_id JOIN academic_year ON academic_year.id = registration.academic_year_id GROUP BY registration.id");
  }
  ...
  public function getDataSupervisorByPeriod($prodi = null)
  {
    return $this->db->query("SELECT academic_year.name as academic_year, company.name as company_name, company.pic, company.norek, company.bank_name, prodi.name as prodi_name, student.fullname, student.npm, registration.group_status FROM registration JOIN supervisor ON supervisor.id = registration.supervisor_id JOIN student ON student.id = registration.student_id JOIN company ON company.id = registration.company_id JOIN prodi ON prodi.id = registration.prodi_id JOIN academic_year ON academic_year.id = registration.academic_year_id");
  }

  public function getDataSupervisorGroupBy($prodi = null)
  {
    return $this->db->query("SELECT academic_year.name as academic_year, company.name as company_name, company.pic, company.norek, company.bank_name,prodi.name as prodi_name, student.fullname, student.npm, registration.group_status FROM registration JOIN supervisor ON supervisor.id = registration.supervisor_id JOIN student ON student.id = registration.student_id JOIN company ON company.id = registration.company_id JOIN prodi ON prodi.id = registration.prodi_id JOIN academic_year ON academic_year.id = registration.academic_year_id GROUP BY registration.id");
  }
